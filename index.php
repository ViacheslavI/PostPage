...
<body>
<h1 class="text-center mt-3 col-md-12">Counter</h1>
<div class="container px-4">
    <div class="row gx-5">
        ...
        <div class="col">
            <div class="p-3 border bg-light">All Posts</div>
        </div>
        <div class="col">
...
<div class="b-popup" id="formAddPost">
    <form action="" class="b-container mt-5 " id="formAddPost">
        <div>
...
<div>
    <form action="" class="b-container mt-5 " id="formAddComment">
        <div>
            <label for="nameAuthor" class="m-3">Name</label>
            <input type="text" id="nameAuthor" placeholder="name">
        </div>
        <div>
            <label for="text" class="m-3">Text</label>
            <textarea name="contextComment" id="contextComment" cols="50" rows="3"></textarea>
        </div>
        <div class="alert alert-danger mt-2" id="errorBlock"></div>
        <div class="d-flex justify-content-end">
            <a class="b-popup-content btn btn-primary" id='AddComment'>Added</a>
        </div>
    </form>
</div>
...
<script>
    ...
    function renderPost() {
        if (($('#name').val() !== '') && $('#context').val() !== '') {
            ...
            min.innerHTML = min.innerHTML + `<div class="card  my-4 w-50"><button onclick="addComment()" class="btn btn-success buut" id="qwe">Add Comment</button><span class="p-3">${name} </span> <p class="p-3"> ${context}</p><span class="text-end p-1"> ${new Date().toDateString()} </span><span  hidden class="id"> ${item["id"]}</span></div>`
        }
    }

    $('#AddComment').click(function () {
        const name = $('#nameAuthor').val()
        const context = $('#contextComment').val()
        const id = $('.id').text()
        console.log(id)
    })
